...
public class CatmullRom {
    ...
    class CatmullRomSpline {
        ...
        protected double q(final double T) {
            double interpolated = 0.5 * ((2 * p1) + (p2 - p0) * T + (2 * p0 - 5 * p1 + 4 * p2 - p3) * T * T + (3 * p1 - p0 - 3 * p2 + p3) * T * T * T);
            // Clamp the value to be within the range [min(p1, p2), max(p1, p2)]
            double minValue = Math.min(p1, p2);
            double maxValue = Math.max(p1, p2);
            return Math.max(minValue, Math.min(interpolated, maxValue));
        }
